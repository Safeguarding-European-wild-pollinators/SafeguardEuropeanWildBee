...
To use the scripts, simply follow these steps:
    ...
    Install the required packages by running source('library.R').
    Prepare your dataset according to the format described in the project's documentation (e.g., .csv or .tsv files).
    Run the individual scripts in the desired order. For example:
        script_data_import.R to import your raw data.
        script_taxonomic_assignment.R for assigning taxonomic ranks.
    Modify the scripts as needed for your specific dataset or research requirements.
