...
df_distribution <- st_as_sf(df_distribution, coords = c("decimalLongitude", "decimalLatitude"), crs = 4326, remove = FALSE)  
# Transformation of the simple feature df_distribution to the ETRS89-extended projection
df_distribution <- st_transform(df_distribution, projection_3035)

# Intersection between df_distribution and europe_3035
# This step will attribute the country and others information to each data point
df_geometry <- st_intersection(df_distribution, europe_3035, remove = FALSE) # /!\ remove = FALSE, prevents to remove decimalLongitude and decimalLatitude
df_geometry <- df_geometry %>% mutate(country = COUNTRY) # Overwrite country column with the COUNTRY column from europe_3035
...
generate_all_species_maps <- function(df_map, output_dir) {
  ...
  cat("=====================================================================================================", "\n", "Start batch map :", "\n")
    ...
    for (i in 1:num_total_species) {
      ...
      generate_species_map(df_map, species_name, csv_dir, xlsx_dir, map_dir)
...
generate_species_map <- function(df_map, species_name, csv_dir, xlsx_dir, map_dir) {
...
output_directory <- paste0("./output/test_", Sys.Date(), "_05_batch_maps")
dir.create(output_directory, showWarnings = FALSE, recursive = TRUE)

# Launch the batch map generation
generate_all_species_maps(df_map, output_directory)
